...
def generate_image_from_scratch(
    ...
    # 1. Initialize the scheduler to define the timestep schedule
    scheduler.set_timesteps(num_inference_steps, device=unet.device)
    
    # 2. Generate initial pure Gaussian noise latents
    latents = torch.randn(
    ...
    uncond_embeddings = torch.zeros_like(text_embedding)
    text_embedding = torch.cat([uncond_embeddings, text_embedding], dim=0)
...
if __name__ == "__main__":
    ...
    text_embedding = get_text_embedding(tokenizer, text_encoder, prompt)
    
    # image = Image.open(in_image_path).convert("RGB")
    # decoded_img = forward_pass_through_pretrained_vae_and_unet(vae, unet, image)
    decoded_img = generate_image_from_scratch(unet, vae, scheduler, text_embedding)
